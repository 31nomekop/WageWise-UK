...
function syncStudentLoanUI(){
  const sel = document.getElementById('studentLoan');
  const help = document.getElementById('help_studentLoan');
  if(!sel) return;

  const ty = getTY();
  const plan5Opt = sel.querySelector('option[value="plan5"]');
  const plan5Supported = !!(ty && ty.slPlan5Threshold != null);

  if(plan5Opt){
    plan5Opt.disabled = !plan5Supported;
    plan5Opt.hidden = !plan5Supported;
    if(!plan5Supported) plan5Opt.setAttribute('hidden', '');
    else plan5Opt.removeAttribute('hidden');
    plan5Opt.setAttribute('aria-hidden', String(!plan5Supported));
    plan5Opt.textContent = plan5Supported ? 'Plan 5' : 'Plan 5 (2026/27+)';
  }

  if(!plan5Supported && sel.value === 'plan5'){
    sel.value = 'none';
  }

  if(help){
    help.innerHTML = plan5Supported
      ? 'Select the student loan plan shown on your payslip. If you’re below the threshold, repayment will be £0.'
      : 'Select the student loan plan shown on your payslip. Plan 5 is only available in WageWise for 2026/27 and later. If you’re below the threshold, repayment will be £0.';
  }
...
function setTaxYear(year){
  ...
  applyTaxYearToUI();
  syncStudentLoanUI();
  requestAnimationFrame(() => syncStudentLoanUI());
...
function normalizeTaxCode(raw){
  ...
  // Strip common regional prefixes because region is chosen separately in the app.
  code = code.replace(/^[SC](?=(NT|BR|D0|D1|0T|K\d+|\d+[A-Z]+))/, '');

  // Remove common emergency/week-month-1 suffixes so the core code still parses.
  code = code.replace(/(W1|M1|X)$/,'');

  return code;
}

function isSupportedTaxCodeFormat(raw){
  const code = normalizeTaxCode(raw);
  return /^(NT|BR|D0|D1|0T|K\d+[A-Z]?|\d+[A-Z]+)$/.test(code);
}
...
function validateAllInputs(){
  ...
  if(presetTC === 'custom' && customTC){
    const ok = isSupportedTaxCodeFormat(customTC);
    if(!ok) setFieldError('taxCodeCustom','err_taxCodeCustom','Check your tax code format (e.g. 1257L, S1257L, BR, 1257L W1).');
  }
